...
(function () {
  ...
  /* -------------------------------------------------------------------------
   * 固定稅則對照表（特定客人 → 固定 HS Code）
   *   規則：品項稅則預設用「FedEx 推薦的第一個」；
   *         只有下列客人命中時，改用這裡的固定稅則，不吃 FedEx 推薦。
   *   命中方式：比對好馬吉「寄件人姓名 / 公司 / 統編」是否包含 key。
   *   等你給規則，把下面兩筆填上即可（可依品名再細分，見註解）。
   * ---------------------------------------------------------------------- */
  const FIXED_HS_BY_CUSTOMER = [
    // 範例：
    // { match: '統一數網',          hsCode: '1234.56.7890' },
    // { match: 'JING SHENGYU',     hsCode: '9876.54.3210' },
    // 若同一客人不同品名要不同稅則，可加 whenProduct：
    // { match: '某客人', whenProduct: '面膜', hsCode: '3304.99.0000' },
  ];
